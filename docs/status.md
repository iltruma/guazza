...
- `guazza-ingest daily` rimosso (comando CLI cancellato in v0.16.0): l'ingestion giornaliera era duplicata 1:1 in `guazza-review` (stessa data, stesso orario). Recupero giorni mancanti → finestra [ieri-7, ieri] di `guazza-review`; backfill Netatmo multi-giorno non più disponibile come comando singolo (solo per-giorno in review).
