...
Schema di una riga:
  ...
  NWP per modello (ecmwf/icon/gfs/arome) × (tmin, tmax, precip, humidity, wind)
...
-- ── 5. Pivot 4 modelli → wide ─────────────────────────────────────────────────
nwp_wide AS (
    SELECT
        location_id,
        target_date,
        lead_time_h,
        MAX(CASE WHEN source = 'open_meteo_ecmwf_ifs025' THEN tmin_c END)       AS ecmwf_tmin_c,
        MAX(CASE WHEN source = 'open_meteo_ecmwf_ifs025' THEN tmax_c END)       AS ecmwf_tmax_c,
        MAX(CASE WHEN source = 'open_meteo_ecmwf_ifs025' THEN precip_mm END)    AS ecmwf_precip_mm,
        MAX(CASE WHEN source = 'open_meteo_ecmwf_ifs025' THEN humidity_pct END) AS ecmwf_humidity_pct,
        MAX(CASE WHEN source = 'open_meteo_ecmwf_ifs025' THEN wind_ms END)      AS ecmwf_wind_ms,
...
SELECT
    ...
    -- Ensemble mean (media sui modelli non-null)
    (COALESCE(n.ecmwf_tmin_c, 0) + COALESCE(n.icon_tmin_c, 0)
        + COALESCE(n.gfs_tmin_c, 0) + COALESCE(n.arome_tmin_c, 0))
    / NULLIF(
    ...
    GREATEST(n.ecmwf_tmin_c, n.icon_tmin_c, n.gfs_tmin_c, n.arome_tmin_c)
        - LEAST(n.ecmwf_tmin_c, n.icon_tmin_c, n.gfs_tmin_c, n.arome_tmin_c)
        AS nwp_tmin_spread,

    (COALESCE(n.ecmwf_tmax_c, 0) + COALESCE(n.icon_tmax_c, 0)
        + COALESCE(n.gfs_tmax_c, 0) + COALESCE(n.arome_tmax_c, 0))
    / NULLIF(
    ...
    GREATEST(n.ecmwf_tmax_c, n.icon_tmax_c, n.gfs_tmax_c, n.arome_tmax_c)
        - LEAST(n.ecmwf_tmax_c, n.icon_tmax_c, n.gfs_tmax_c, n.arome_tmax_c)
        AS nwp_tmax_spread,

    (COALESCE(n.ecmwf_precip_mm, 0) + COALESCE(n.icon_precip_mm, 0)
        + COALESCE(n.gfs_precip_mm, 0) + COALESCE(n.arome_precip_mm, 0))
    / NULLIF(
    ...
    GREATEST(n.ecmwf_precip_mm, n.icon_precip_mm, n.gfs_precip_mm, n.arome_precip_mm)
        - LEAST(n.ecmwf_precip_mm, n.icon_precip_mm, n.gfs_precip_mm, n.arome_precip_mm)
